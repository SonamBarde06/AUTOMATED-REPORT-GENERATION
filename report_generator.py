...
class PDFReport(FPDF):
    def header(self):
        self.set_fill_color(0, 102, 204)  # Bright blue background
    ...
    def add_graph(self, graph_path):
        ...
        img_height = img_width * 0.6  # Adjust this ratio based on your graph's aspect

        # Get current Y position and center the image
...
def create_graph(city_crime_data, output_filename):
    ...
    plt.bar(x, violent_crimes, width=bar_width, label='Violent Crime', color='#e74c3c', align='center')
    plt.bar([p + bar_width for p in x], property_crimes, width=bar_width, label='Property Crime', color='#3498db', align='center')
    plt.bar([p + bar_width * 2 for p in x], drug_offenses, width=bar_width, label='Drug Offenses', color='#2ecc71', align='center')
